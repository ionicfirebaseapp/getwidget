...
/// The rendering mode of the [GFSlidable]
enum GFSlidableRenderingMode {
  /// The [GFSlidable] is not showing any actions.
  none,
...
typedef void DismissSlideActionCallback(GFSlideActionType actionType);

/// Signature for determining whether the widget will be dismissed for the
/// given [actionType].
///
/// Used by [SlideToDismissDelegate.onWillDismiss].
typedef FutureOr<bool> SlideActionWillBeDismissed(GFSlideActionType actionType);
...
class GFSlidableData extends InheritedWidget {
  ...
  /// The type of slide action that is currently been showed by the [GFSlidable].
  final GFSlideActionType actionType;
  ...
  bool get showActions => actionType == GFSlideActionType.primary;

  /// The number of actions.
  int get actionCount => actionDelegate?.actionCount ?? 0;

  /// If the [actionType] is [GFSlideActionType.primary] returns 1, -1 otherwise.
  double get actionSign => actionType == GFSlideActionType.primary ? 1.0 : -1.0;
  ...
  static GFSlidableData of(BuildContext context) => context.inheritFromWidgetOfExactType(GFSlidableData);

  /// Gets the the given offset related to the current direction.
  Offset createOffset(double value) => directionIsXAxis ? Offset(value, 0) : Offset(0, value);


  /// Gets the maximum extent in the current direction.
  double getMaxExtent(BoxConstraints constraints) => directionIsXAxis ? constraints.maxWidth : constraints.maxHeight;
...
//    );
  ...
  List<Widget> buildActions(BuildContext context) => List.generate(
      actionCount,
...
//          ..addListener(_handleOverallPositionChanged);
  ...
  GFSlideActionType _actionType = GFSlideActionType.primary;
  GFSlideActionType get actionType => _actionType;
  set actionType(GFSlideActionType value) {
    _actionType = value;
    _initAnimations();
  }
...
//  bool get _dismissible => widget.dismissal != null && _dismissThreshold < 1.0;
  ...
  SlideActionDelegate get _actionDelegate =>
      actionType == GFSlideActionType.primary
  ...
  double get _actionsDragAxisExtent => _overallDragAxisExtent * _totalActionsExtent;
  ...
  /// By default it's open the [GFSlideActionType.primary] action pane, but you
  /// can modify this by setting [actionType].
  void open({GFSlideActionType actionType}) {
  ...
  /// By default it's dismiss by showing the [GFSlideActionType.primary] action pane, but you
  /// can modify this by setting [actionType].
//  void dismiss({GFSlideActionType actionType}) {
//    if (_dismissible) {
//      _dismissing = true;
...
//          ? GFSlideActionType.primary
//          : GFSlideActionType.secondary;
//      if (_actionCount > 0) {
...
//  }
  ...
  Widget build(BuildContext context) {
    ...
    if (!(!widget.enabled ||
      ...
      if (actionType == GFSlideActionType.primary &&
              widget.actionDelegate != null &&
              widget.actionDelegate.actionCount > 0 ||
          actionType == GFSlideActionType.secondary &&
              widget.secondaryActionDelegate != null &&
              widget.secondaryActionDelegate.actionCount > 0) {
//        if (_dismissible) {
